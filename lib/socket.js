...
var opts = {};
Object.keys(binding.options).forEach(function (name) {
  var shortName = name.replace(/^ZMQ_/, '').toLowerCase();

  opts[name] = opts[shortName] = binding.options[name];
});
...
var types = {};
Object.keys(binding.types).forEach(function (name) {
  var shortName = name.replace(/^ZMQ_/, '').toLowerCase();

  types[name] = types[shortName] = binding.types[name];
});
...
var sendFlags = {};
Object.keys(binding.sendFlags).forEach(function (name) {
  var shortName = name.replace(/^ZMQ_/, '').toLowerCase();

  sendFlags[name] = sendFlags[shortName] = binding.sendFlags[name];
});
...
function Socket(type, ctx) {
  ...
  this.type = type;
  this._sock = new binding.SocketBinding(ctx._ctx, types[type]);
  this._paused = false;
...
/**
 * Ref the socket on the event-loop. A no-op if already ref()ed
 *
 * @return {Socket} for chaining
 */
...
/**
 * Unref the socket from the event-loop. A no-op if already unref()ed
 *
 * @return {Socket} for chaining
 */
...
Socket.prototype.set = function (opt, val) {
  ...
  this._sock.setsockopt(opts[opt], val);
  return this;
};
...
Socket.prototype.monitor = function (interval, numOfEvents) {
  if (!this._sock.monitor) {
    throw new Error('Monitoring support disabled. Ensure ZMQ version is > 3.2.1 and recompile node-binding.');
  }
...
Socket.prototype.unsubscribe = function (filter) {
  this.set('ZMQ_UNSUBSCRIBE', filter);
  return this;
};
