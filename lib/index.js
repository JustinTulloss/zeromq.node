...
Socket.prototype.send_multi = function(msgs) {
  var self = this;
  msgs.forEach(function(msg,i) {
    if (i == msgs.length - 1) self.send(msg);
    else self.send(msg,zmq.ZMQ_SNDMORE);
  })
  return this;
}
  ...
  try {
    while (true) {
      ...
      if (flags & zmq.ZMQ_POLLIN) {
          ...
          // Fixes bug that occured when a message was received in the middle of a send_multi call.
          (function(emitArgs) {
