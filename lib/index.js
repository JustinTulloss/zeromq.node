...
exports.Socket = function (type, options) {
  ...
  this._zmq.onReady = function() {
    ...
    if (self.autoFlush === false) return;
...
Socket.prototype.bind = function(addr, cb) {
  ...
  self._zmq.bind(addr, function(err) {
    try {
      if (self.autoFlush !== false) self._flush();
      self.emit('bind', addr);
      cb && cb(err);
    } catch (err) {
      self.emit('error', err);
 ...
 *
 * @return {Array} multi-part message or null if socket doesn't have any
 * @api public
 */
 ...
 * @api public
 */
Socket.prototype.write = function (parts, callback) {
  var self = this,
    i = 0,
    len = parts.length,
    flags = this._zmq.getsockopt(zmq.ZMQ_EVENTS),
  ...
  // socket is not ready to accept messages
  if (!(flags & zmq.ZMQ_POLLOUT)) {
    server.once('ready', function () {
      self.write(parts, callback);
    });
    return;
  }
  ...
  while (i < len) {
    try {
      ...
      if (i === len - 1) {
        this._zmq.send(part, 0);
      } else {
        this._zmq.send(part, zmq.ZMQ_SNDMORE);
      }
    } catch (sendError) {
      ...
      if (callback) return callback(sendError);
    }
...
exports.createSocket = function(type, options) {
  var sock = new Socket(type, options);
  return sock;
};
