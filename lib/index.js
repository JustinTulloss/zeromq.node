...
var Socket =
exports.Socket = function (ctx, type) {
  var self = this;
  ...
  if (! (type in types)) {
    throw new TypeError('unrecognized socket type ' + type);
  }
  this.type = type;
  this._zmq = new zmq.SocketBinding(ctx, types[type]);
...
exports.socket =
exports.createSocket = function(ctx, type, options) {
  var sock = new Socket(ctx, type);
  if (typeof ctx === 'string') {
    options = type;
  }
