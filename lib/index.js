  ...
  try {
    while (true) {
      ...
      if (flags & zmq.ZMQ_POLLIN) {
          ...
          emitArgs = ['message'].concat(emitArgs.map(this.restore))
...
Socket.prototype.transform = function (buf, idx, arr) {
  return buf;
};
...
Socket.prototype.restore = function (buf, idx, arr) {
  return buf;
};
